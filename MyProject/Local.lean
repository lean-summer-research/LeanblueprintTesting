...
structure SubsemigroupBundled (α : Type*) [Mul α] where
  carrier : Set α
  mul_mem {x y : α} (hx : x ∈ carrier) (hy : y ∈ carrier) : x * y ∈ carrier
  mul_assoc {x y z : α} (hx : x ∈ carrier) (hy : y ∈ carrier) (hz : z ∈ carrier) :
    x * y * z = x * (y * z)

namespace SubsemigroupBundled

variable {α : Type*} [Mul α] {x : α}

/-! This regesters a coersion from `p : SubsemigroupBundeled α` to `Set α`.
This should be accessed through `(p: Set α)`-/
instance : SetLike (SubsemigroupBundled α) α where
  coe := SubsemigroupBundled.carrier
  coe_injective' := fun p q h => by cases p; cases q; congr!

@[simp] lemma mem_carrier {p : SubsemigroupBundled α} : x ∈ p.carrier ↔ x ∈ (p : Set α) := Iff.rfl

@[ext] theorem ext {p q : SubsemigroupBundled α} (h : ∀ x, x ∈ p ↔ x ∈ q) : p = q := SetLike.ext h

@[simp] theorem mem_mk {s : Set α} {x : α} (h_mul) (h_assoc) : x ∈ (mk s h_mul h_assoc) ↔ x ∈ s :=
  Iff.rfl

@[simp]
theorem coe_set_mk (s : Set α) (h_mul) (h_assoc) : (mk s h_mul h_assoc: Set α) = s :=
  rfl

@[simp]
theorem mk_le_mk {s t : Set α} (h_mul) (h_mul') (h_assoc) (h_assoc') :
    mk s h_mul h_assoc ≤ mk t h_mul' h_assoc' ↔ s ⊆ t := Iff.rfl

-- lower priority so other instances are found first
/-- A submagma of a magma inherits a multiplication. -/

instance (priority := 900) mul (p : SubsemigroupBundled α) : Mul (p : Set α) :=
  ⟨fun a b => ⟨a.1 * b.1, p.mul_mem a.2 b.2⟩⟩

-- lower priority so later simp lemmas are used first; to appease simp_nf
@[simp]
theorem coe_mul {p : SubsemigroupBundled α} (x y : (p : Set α)) : (↑(x * y) : α) = ↑x * ↑y :=
  rfl

-- lower priority so later simp lemmas are used first; to appease simp_nf
@[simp]
theorem mk_mul_mk {p : SubsemigroupBundled α} {x y : α} (hx : x ∈ p) (hy : y ∈ p) :
    (⟨x, hx⟩ : (p : Set α)) * ⟨y, hy⟩ = ⟨x * y, p.mul_mem hx hy⟩ :=
  rfl

theorem mul_def {p : SubsemigroupBundled α} (x y : (p : Set α)) : x * y = ⟨x * y, p.mul_mem x.2 y.2⟩ :=
  rfl

instance isSemigroup (p : SubsemigroupBundled α) : Semigroup (p : Set α) where
  toMul := p.mul
  mul_assoc (x y z : (p : Set α)) := by
    rw [← Subtype.coe_inj, coe_mul, coe_mul, coe_mul, coe_mul]
    apply p.mul_assoc <;> simp

end SubsemigroupBundled

structure SubmonoidBundled (α : Type*) [Mul α] extends SubsemigroupBundled α where
  one : α
  one_mem : one ∈ carrier
  one_mul {x : α} (hx : x ∈ carrier) : one * x = x
  mul_one {x : α} (hx : x ∈ carrier) : x * one = x

namespace SubmonoidBundled

variable {α : Type*} [Mul α] {x : α}

instance : SetLike (SubmonoidBundled α) α where
  coe (p : SubmonoidBundled α):= p.carrier
  coe_injective' := fun p q h => by
    rcases p with ⟨hSemigroupP, one₁, one_mem₁, one₁_mul, mul_one₁⟩
    rcases q with ⟨hSemigroupQ, one₂, one_mem₂, one₂_mul, mul_one₂⟩
    cases hSemigroupP
    cases hSemigroupQ
    congr
    simp_all
    simp_all
    have h₁ : one₁ * one₂ = one₁ := by
      apply mul_one₂
      exact one_mem₁
    have h₂ : one₁ * one₂ = one₂ := by
      apply one₁_mul
...
@[simp] lemma mem_carrier {p : SubmonoidBundled α} : x ∈ p.carrier ↔ x ∈ (p : Set α) := Iff.rfl

@[ext] theorem ext {p q : SubmonoidBundled α} (h : ∀ x, x ∈ p ↔ x ∈ q) : p = q := SetLike.ext h

-- lower priority so other instances are found first
/-- A submagma of a magma inherits a multiplication. -/

instance (priority := 900) mul (p : SubmonoidBundled α) : Mul (p : Set α) :=
  ⟨fun a b => ⟨a.1 * b.1, p.mul_mem a.2 b.2⟩⟩

-- lower priority so later simp lemmas are used first; to appease simp_nf
@[simp]
theorem coe_mul {p : SubmonoidBundled α} (x y : (p : Set α)) : (↑(x * y) : α) = ↑x * ↑y :=
  rfl

-- lower priority so later simp lemmas are used first; to appease simp_nf
@[simp]
theorem mk_mul_mk {p : SubmonoidBundled α} {x y : α} (hx : x ∈ p) (hy : y ∈ p) :
    (⟨x, hx⟩ : (p : Set α)) * ⟨y, hy⟩ = ⟨x * y, p.mul_mem hx hy⟩ :=
  rfl

theorem mul_def {p : SubmonoidBundled α} (x y : (p : Set α)) : x * y = ⟨x * y, p.mul_mem x.2 y.2⟩ :=
  rfl

instance hasOne (p : SubmonoidBundled α) : One (p : Set α) where
  one := ⟨p.one, p.one_mem⟩

theorem one_def (p : SubmonoidBundled α) : (1 : (p : Set α)) = ⟨p.one, p.one_mem⟩ := rfl

@[simp] theorem coe_one (p : SubmonoidBundled α) : ↑(1 : (p : Set α)) = p.one := rfl

instance isMonoid (p : SubmonoidBundled α) : Monoid (p : Set α) where
  toSemigroup := p.toSubsemigroupBundled.isSemigroup
  toOne := p.hasOne
  one_mul (x : (p : Set α)) := by
    rw [← Subtype.coe_inj, coe_mul]
    simp
    apply p.one_mul x.2
  mul_one (x : (p : Set α)) := by
    rw [← Subtype.coe_inj, coe_mul]
    simp
    apply p.mul_one x.2

end SubmonoidBundled

structure SubgroupBundled (α : Type*) [Mul α] extends SubmonoidBundled α where
  inv {x : α} (hx : x ∈ carrier) : α
  inv_mem {x : α} (hx : x ∈ carrier) : inv hx ∈ carrier
  inv_mul_cancel {x : α} (hx : x ∈ carrier) : (inv hx) * x = one
  mul_inv_cancel {x : α} (hx : x ∈ carrier) : x * (inv hx) = one

namespace SubgroupBundled

variable {α : Type*} [Mul α] {x : α}

instance : SetLike (SubgroupBundled α) α where
  coe (p : SubgroupBundled α) := p.carrier
  coe_injective' := fun p q h => by sorry

@[simp] lemma mem_carrier {p : SubgroupBundled α} : x ∈ p.carrier ↔ x ∈ (p : Set α) := Iff.rfl

@[ext] theorem ext {p q : SubgroupBundled α} (h : ∀ x, x ∈ p ↔ x ∈ q) : p = q := SetLike.ext h

-- lower priority so other instances are found first
/-- A submagma of a magma inherits a multiplication. -/

instance (priority := 900) mul (p : SubgroupBundled α) : Mul (p : Set α) :=
  ⟨fun a b => ⟨a.1 * b.1, p.mul_mem a.2 b.2⟩⟩

-- lower priority so later simp lemmas are used first; to appease simp_nf
@[simp]
theorem coe_mul {p : SubgroupBundled α} (x y : (p : Set α)) : (↑(x * y) : α) = ↑x * ↑y :=
  rfl

-- lower priority so later simp lemmas are used first; to appease simp_nf
@[simp]
theorem mk_mul_mk {p : SubgroupBundled α} {x y : α} (hx : x ∈ p) (hy : y ∈ p) :
    (⟨x, hx⟩ : (p : Set α)) * ⟨y, hy⟩ = ⟨x * y, p.mul_mem hx hy⟩ :=
  rfl

theorem mul_def {p : SubgroupBundled α} (x y : (p : Set α)) : x * y = ⟨x * y, p.mul_mem x.2 y.2⟩ :=
  rfl

instance hasOne (p : SubgroupBundled α) : One (p : Set α) where
  one := ⟨p.one, p.one_mem⟩

theorem one_def (p : SubgroupBundled α) : (1 : (p : Set α)) = ⟨p.one, p.one_mem⟩ := rfl

@[simp] theorem coe_one (p : SubgroupBundled α) : ↑(1 : (p : Set α)) = p.one := rfl

instance isGroup (p : SubgroupBundled α) : Group (p : Set α) where
  toMonoid := p.isMonoid
  inv (x : (p : Set α)) := ⟨p.inv x.2, p.inv_mem x.2⟩
  inv_mul_cancel (x : (p : Set α)) := by
    rw [← Subtype.coe_inj]
    rw [coe_mul]
    simp
    apply p.inv_mul_cancel x.2
...
theorem Subtype.coe_mul {s : Set α} (inst : Subsemigroup' s) (x y : ↑s) : (x * y : ↑s) = ⟨x.val * y.val, inst.mul_mem x.val y.val x.prop y.prop⟩ := by
  rfl
...
example {s : Set α} (inst : Submonoid' s) [One ↑s] : (1 : ↑s) = ⟨inst.one, inst.one_mem⟩ := rfl

-- Question: Why is (1 : ↑s) not definitionally equal to inst.one?
...
def make_SubsemigroupBundled {e : M} (h_idem : IsIdempotentElem e) : SubsemigroupBundled M where
  carrier := ⟦e⟧𝓗
  mul_mem := by sorry
  mul_assoc := by intros; apply mul_assoc

def make_SubmonoidBundled {e : M} (h_idem : IsIdempotentElem e) : SubmonoidBundled M where
  toSubsemigroupBundled := make_SubsemigroupBundled h_idem
  one := e
  one_mem := by
    simp_all [make_SubsemigroupBundled]
  one_mul := by sorry
  mul_one := by sorry
...
instance isSubmonoid' : Submonoid' (HEquiv.set x) where
  ...
  mul_one z hz := by
    ...
    simp_all
    have h₁ : z ≤𝓛 x := by simp_all
    have h₂ : x ≤𝓛 inst.idem := by
      have h₃ := inst.idem_in
      simp_all
    apply LLE.trans h₁ h₂
    exact inst.hidem
...
/- TODO: helper lemma for below
Given some H-class with idempotent `⟦x⟧𝓗`, and `y z ∈ ⟦x⟧𝓗`, and `u * y = z`,
must `u ∈ ⟦x⟧𝓗`?
...
--TODO: generalize below, all elements of the h class
...
-- Equivilant condition: there exist an xy in the hclass such that x * y is in H
...
-- REE MAtrix: define the semigroup of a Regular J class and {0}
